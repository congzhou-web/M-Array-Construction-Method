...
PiecesGrowing::PiecesGrowing(int symbols, int winrow, int wincol) : _win_symbols(symbols), _win_row(winrow), _win_col(wincol), _step(winrow * wincol), _win_sum(pow(_win_symbols, _step)), _errFlag(false)
{	
	//_array_col = _array_row = sqrt((double)_win_sum );
	//_array_col = _array_row = sqrt((double)_win_sum / 2) - 1  ;
	//_array_col = _array_row = sqrt((double)_win_sum / 3) ;
	//_array_col = _array_row = sqrt((double)_win_sum / 4 ) ;

	_array_col = _array_row = 500;

	_thresholdArea = (_win_row  * _win_col )/ 2 ;
	_thresholdEdge = (2 * _win_row  * _win_col - _win_row - _win_col) / 2  ;    //max edge count is 2mn-m-n
...
//从xml读取本原多项式信息
vector<int> PiecesGrowing::xmlParse(int step)
{
...
//M序列构造
bool PiecesGrowing::m_sequenceGenerating()
{
	{
...
//取一个符号用于拼片
inline int PiecesGrowing::getOneValue()
{
	//static int index = 0;
	//return _m_sequence[(++index) %= (_win_sum - 1)];

	return rand() % _win_symbols;

}
//初始化数组
void PiecesGrowing::dataInit()
{
...
//入口主函数
bool PiecesGrowing::algorithmRun()
{
	dataInit();
...
void PiecesGrowing::MArrayFileSave()
{
	if (_errFlag) return;
...
//初始化卷积核
void PiecesGrowing::convKernelInit()
{
...
//卷积窗口码字，传入窗口右下角的坐标
int PiecesGrowing::convWindow(int rowInd, int colInd)
{
...
//根据直方图计算面积
int PiecesGrowing::calArea(vector<int>& heights)
{
...
bool PiecesGrowing::areaCheck(int & rowInd, int & colInd)
{
...
//验证窗口内边缘数量
bool PiecesGrowing::edgeCheck(int & rowInd, int & colInd)
{
...
//验证窗口特性
bool PiecesGrowing::winPropertyCheck(int & rowInd, int & colInd)
{
	if (rowInd < _win_row - 1 || colInd < _win_col - 1) return true;
...
//按行回滚 cnt 个符号
bool PiecesGrowing::rollbackOnRow(int & rowInd, int & colInd, int cnt)
{
